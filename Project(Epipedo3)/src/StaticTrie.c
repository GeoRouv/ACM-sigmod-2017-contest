...
void StaticTrieSizeStatisticsPrivate(HyperNode* n, int* bytes, int* nodes)
{
	*bytes += strlen(n->word)+1;
	*bytes += 3*sizeof(int);
	*bytes += n->arraySize*sizeof(struct HyperNode);
	*bytes += n->lengths_size*sizeof(short);
	(*nodes)++;

	int i;
	for(i = 0; i < n->numChildren; i++)
		StaticTrieSizeStatisticsPrivate(&(n->children[i]), bytes, nodes);
}

void StaticTrieSizeStatistics(void* trie, int* bytes, int* nodes)
{
	StaticTrie ST = (StaticTrie) trie;
	StaticLinearHash L = (StaticLinearHash) (ST->root->children);

	*bytes = sizeof(HyperNode*)+sizeof(int);
	*nodes = 0;

	StaticBucket* buckets = L->buckets;

	int i,j;

	for(i = 0; i < L->curr_m; i++) {
		for(j = 0; j < buckets[i].arr_entries; j++) {
			StaticTrieSizeStatisticsPrivate(&(buckets[i].array[j]), bytes, nodes);
		}
	}
}
